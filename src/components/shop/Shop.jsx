...
export default function Shop() {
  ...
  const { data: userCards, refetch: refetchUserCards } = useFetch(shopService.getUserCards);
  ...
  if (error) return (<div className="flex justify-center items-center min-h-screen">{t('shop.serverError')}</div>);

  return (
    <div className="pt-[50px] pb-[150px]">
      <UserStats />
      <div className="max-w-[350px] mx-auto">
        <h2 className="text-3xl font-bold text-white mb-[30px] tracking-wider">{t('shop.title')}</h2>
        <div className="border-2 border-[#292139] rounded-md bg-[#D9D9D9] py-[45px] px-[20px]">
          ...
          <p className="text-center mb-5 font-semibold text-xl">
            {t('shop.collected', { current: userCards?.length, total: CARDS_NUMBER })}
          </p>
