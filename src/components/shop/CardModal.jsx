...
export default function CardModal({cardId, onConfirm, onClose}){
    ...
    function getBonusString(type){
        ...
        if(type === 'currency_per_habit') return 'coins per completed habit'
    ...
    return(
        <div className="flex items-center justify-center min-h-screen fixed inset-0" style={{backgroundColor: "rgba(0,0,0,0.6"}}>
            <div className="flex flex-col items-center p-[25px] pt-[60px] pb-[45px] min-w-[250px] min-h-[350px] bg-white rounded-md shadow-lg relative">
                <p className="text-2xl max-w-[270px] text-center mb-[30px] font-bold">Would you like to buy "{cardData.name}"?</p>
                <img src={card2} alt="Card" className="min-w-[150px] mb-8"/>
                <p className="max-w-[275px] text-center mb-5 font-semibold text-xl">+ {cardData?.bonus_value} {getBonusString(cardData?.bonus_type)}</p>
                <button onClick={onConfirm} className="flex gap-2 items-center bg-[#FFFAA3] px-8 border-2 border-[#563897] rounded-full shadow-xl">
