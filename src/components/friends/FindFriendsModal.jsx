...
export default function FindFriendsModal({onClose}){
    ...
    return(
        <div className="flex items-center justify-center min-h-screen fixed inset-0" style={{backgroundColor: "rgba(0,0,0,0.7)"}}>
            <div className="bg-[#D9D9D9] relative w-full max-w-[350px] px-8 py-12 border-2 border-[#292139] rounded-md min-h-[500px]">
                <p className="text-center font-semibold text-2xl mb-5">Find new friends!</p>
                <form>
                    <div className="relative mb-5">
                        <input className="border-2 border-[#292139] w-full rounded-sm shadow-lg text-lg pl-3 pr-15 py-2 bg-white"
                            name="input" id="input" type="text"
                            value={inputValue} onChange = {handleChange} placeholder="Enter here"/>
                        <button onClick={handleSearch} className="absolute right-0 h-full border-2 w-[50px] rounded-sm bg-[#382c9c]">
                ...
                {foundUsers.length === 0 ?
                    ...
                    : 
                    foundUsers?.map((user)=>(
                        <li className="flex flex-col gap-5 bg-white border-2 border-[#292139] rounded-md shadow-lg px-6 py-3">
