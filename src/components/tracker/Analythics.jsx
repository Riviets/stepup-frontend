  ...
  return (
    <div className="flex items-center justify-center fixed inset-0" style={{ backgroundColor: "rgba(0,0,0,0.6)" }}>
      <div className="px-6 py-8 bg-[#D9D9D9] border-2 border-[#483D61] rounded-md w-full max-w-[370px] max-h-[600px] overflow-scroll relative">
        ...
        {isLoading && <div className="text-center py-4">{t('analytics.loading')}</div>}
        {error && <div className="text-center py-4 text-red-500">{t('analytics.error')}</div>}
        {analyticsData ? (
          <div className="flex flex-col gap-6">
            <div className="bg-white p-4 rounded-lg border border-[#563897]">
              <h2 className="text-lg font-semibold mb-3">{t('analytics.dailyCompletions')}</h2>
              <div className="h-64">
            ...
            <div className="bg-white p-4 rounded-lg border border-[#563897]">
              <h2 className="text-lg font-semibold mb-3">{t('analytics.totalStats')}</h2>
            ...
            {analyticsData.topHabit && (
              <div className="bg-white p-4 rounded-lg border border-[#563897]">
                <h2 className="text-lg font-semibold mb-3">{t('analytics.topHabit')}</h2>
                <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-md border border-[#292139]">
                  <div>
                    ...
                    <p className="text-sm">
                      {t('analytics.completedTimes', { count: analyticsData.topHabit.completions })}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <button onClick={onClose} className="absolute top-5 right-7">
          <img src={close} alt="Close Analytics Modal" className="min-w-[15px]" />
        </button>
      </div>
    </div>
  );
