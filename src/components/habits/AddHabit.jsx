...
export default function AddHabit() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      await habitsService.createUserHabit(habitValues);
      setHabitValues(defaultValues);
      setMessage(t("addHabit.habitAdded"));
      setIsModalOpen(true);
    } catch (error) {
  ...
  return (
    <div className="px-[50px] pt-[60px]">
      ...
      <div className="bg-[#D9D9D9] w-[100%] max-w-[330px] border-2 border-[#483D61] rounded-lg py-10 px-8">
        <p className="text-center text-2xl font-black mb-5">
          {t("addHabit.title")}
        </p>
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="form-item">
            <label className="text-lg font-semibold" htmlFor="text">
              {t("addHabit.habitTextLabel")}
            </label>
            <input
              className="input"
              type="text"
            ...
            />
            {formErrors.text && (
              <div className="text-red-500">{formErrors.text}</div>
            )}
          </div>
          <div className="form-item">
            <label className="text-lg font-semibold" htmlFor="xp">
              {t("addHabit.xpLabel")}
            </label>
            <input
              className="input"
          ...
          <div className="form-item mb-5">
            ...
            />
            {formErrors.currency && (
              <div className="text-red-500">{formErrors.currency}</div>
            )}
          </div>
          <Button
            type="submit"
            className="bg-[#483D61] text-white py-2 rounded-lg text-xl font-bold"
